...
| **- Enable DLSS**               | **NOTE: only available if the NVIDIA native package is enabled in the package manager**. Enable the checkbox to make HDRP support NVIDIA Deep Learning Super Sampling (DLSS). |
| **-- Performance Quality**      | Use the drop-down to choose which performance mode DLSS will operate onto. |
| **-- Use Optimal Settings**     | Enable the checkbox to make DLSS control the Sharpness and Screen Percentage automatically. |
| **-- Sharpness**                | Use the slide-bar to set the pixel sharpness of the DLSS upscaler. |
