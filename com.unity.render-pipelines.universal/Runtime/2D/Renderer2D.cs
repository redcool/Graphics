...
namespace UnityEngine.Experimental.Rendering.Universal
{
    internal class Renderer2D : ScriptableRenderer
    {
        ...
        public Renderer2D(Renderer2DData data) : base(data)
        {
            ...
            k_ColorTextureHandle = RTHandles.Alloc(Shader.PropertyToID("_CameraColorTexture"), "_CameraColorTexture");
            k_DepthTextureHandle = RTHandles.Alloc(Shader.PropertyToID("_CameraDepthAttachment"), "_CameraDepthAttachment");
            ...
            out RTHandle depthTargetHandle)
        {
            ...
            if (cameraData.renderType == CameraRenderType.Base)
            {
                ...
                if (m_CreateColorTexture)
                {
                    ...
                    cmd.GetTemporaryRT(Shader.PropertyToID(k_ColorTextureHandle.name), colorDescriptor, colorTextureFilterMode);
                }
                ...
                if (m_CreateDepthTexture)
                {
                    ...
                    cmd.GetTemporaryRT(Shader.PropertyToID(k_ColorTextureHandle.name), depthDescriptor, FilterMode.Point);
                }
            }
            else    // Overlay camera
            {
        ...
        {
            if (m_CreateColorTexture)
                cmd.ReleaseTemporaryRT(Shader.PropertyToID(k_ColorTextureHandle.name));

            if (m_CreateDepthTexture)
                cmd.ReleaseTemporaryRT(Shader.PropertyToID(k_DepthTextureHandle.name));
        }
    }
}
